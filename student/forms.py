...
class StudentRegistrationForm(forms.Form):
    birth_date = forms.CharField(max_length=12,required=False, label='Date Of Birth', widget=forms.TextInput(attrs={'readonly': 'readonly'}))
    ...
    birth_place = forms.CharField(label= 'Place of Birth', max_length=75,required=False)
    ...
    fathername = forms.CharField(label= "Name", max_length=275,required=False)
    fatheraddress = forms.CharField(label= 'Address', max_length=200,required=False)
    fathernumber = forms.CharField(label= 'Phone Number', max_length=35,required=False)#validators=[PhoneNumberValidator]s
    fatheroccupation = forms.CharField(label= 'Occupation', required=False,max_length=175)
    ...
    admitted_arm = forms.ChoiceField(label= 'Room', choices = [(a.arm, a.arm) for a in Arm.objects.all()])
    ...
    subclass = forms.ChoiceField(label= 'Sub Class Admitted',choices=(('Nil', 'Nil'), ('JS', 'JS'), ('Science', 'Science'),('Art', 'Art'),('Commercial', 'Commercial'),('Remidials', 'Remidials')))
...
class klassreportform(forms.Form):
    session = forms.CharField(label='Session',max_length= 190,required=True,widget= forms.TextInput(attrs ={'size':'10'}),initial=currse)
    klass = forms.ChoiceField(label= 'Class',choices = [(c.klass, c.klass) for c in Class.objects.all()])
    arm = forms.ChoiceField(label= 'Arm', choices = [(a.arm, a.arm) for a in Arm.objects.all()])
    ...
    def __init__(self, *args):
         super(klassreportform, self).__init__(*args)
         self.fields['arm'].choices = [(a.arm, a.arm) for a in Arm.objects.all()]
         self.fields['arm'].initial = Arm.objects.all()
         self.fields['klass'].choices = [(c.klass, c.klass) for c in Class.objects.all()]
         self.fields['klass'].initial = Class.objects.all()
