...
#     School(name='The Salvation Army',address='2, Shipeolu Street, Onipanu, somolu Lagos',logo="C:/Users/CCL/Desktop/Salvation Army/static/school-logo/log.jpeg").save()
...
#     tt = ['Mid term', 'End term']
#     for k in tt:
#         if k == 'End term':
#             status ='ACTIVE'
#         else:
#             status='INACTIVE'
#         tblreportsheet(reportsheet=k, status=status).save()
...
#             tblterm(term=t,creator='admin',status= status).save()
...
#     subclass(subcode='S',classsub='Remidials').save()
...
# ##current Session
#     currentsession(session='2024/2025').save()
...
def welcomecode(request):
    if  "userid" in request.session:
        ...
        if request.method == 'POST':
           ...
           # varuser=[]
           form = SearchForm()
        #   msg ='Maintenance is on-going, pls check back in an hour. Thank you'
        #   return render_to_response('selectloan.html',{'msg':msg})
        #   msg = 'It appears your hosting plan has reached its limit, Pythonanywhere will no longer serve pages from this App. Contact the Admin'
        #   return render_to_response('selectloan.html',{'msg':msg})
...
def speech(request):
    ...
    if  "userid" in request.session:
        ...
        getdetails =""
        if request.method == 'POST':
            form = speechform(request.POST) # A form bound to the POST data
            if form.is_valid():
                session = form.cleaned_data['session']
                klass = form.cleaned_data['klass']
                scores=[]
                d = StudentAnnualRecord.objects.filter(session=session,klass=klass).order_by('annualaverage').reverse()[:20]
                return render_to_response('sysadmin/speech.html',{'varuser':varuser,'varerr':varerr,'form':form,'getdetails':d},context_instance = RequestContext(request))
        else:
            form = speechform()



        return render_to_response('sysadmin/speech.html',{'varuser':varuser,'varerr':varerr,'form':form},context_instance = RequestContext(request))
    ...
    if  "userid" in request.session:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                getdetails = userprofile.objects.filter(status='ACTIVE').order_by('staffname')
                varerr='Successfull'
                return render_to_response('sysadmin/createuser.htm',{'varuser':varuser,'varerr':varerr,'form':form})
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    varvali.save()
                    #********************************************************
                    from hrm.rcwadmin.models import tbluseracc as hrmuser
    ...
    if  "userid" in request.session:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                teachername = str(teachername1).lower()
                if ClassTeacher.objects.filter(teachername=teachername,klass = klass,arm = arm):
                    varerr ="Class Already Taken By Teacher !"
...
def deleteduration(request,invid):
    ...
    if  "userid" in request.session:
        ...
        if request.method=='POST':
            ...
            k = StudentAcademicRecord.objects.filter(session=currse,term=term)
            k.update(days_open=duration)
            for jj in k:
                jj.days_present = int(jj.days_open) - int(jj.days_absent)
                jj.save()
    ...
    if  "userid" in request.session:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                tttt= userprofile.objects.filter(username=teachername)
                ttc=tttt.count()
                ...
                if ttc == 1 :
                    ...
                    userprofile.objects.filter(username=teachername).update(reportsheet=1)
...
def population(request):
    ...
    if  "userid" in request.session:
        ...
        return render_to_response('sysadmin/population.html',{'varuser':varuser,'form':form},context_instance = RequestContext(request))

    else:
        return HttpResponseRedirect('/login/')


def getschoolpop(request):
    if  "userid" in request.session:
        if request.is_ajax():
            if request.method == 'POST':
                varuser = request.session['userid']
                ...
                for k in klass:
                    ...
                    if term == 'First':
                        sd = Student.objects.filter(admitted_session=currse,gone=0, first_term=1,admitted_class=k)
                        count= sd.count()

                        gh={'klass':k, 'total':count}
                        tty.append(gh)
                        total=total+ count

                    elif term == 'Second':
                        sd = Student.objects.filter(admitted_session=currse,gone=0, second_term=1,admitted_class=k)
                        count= sd.count()
                        ...
                        sd = Student.objects.filter(admitted_session=currse,gone=0, third_term=1,admitted_class=k)
                        count= sd.count()
                ...
                return render_to_response('sysadmin/stupop.html',{'total':total,'getdetails':tty,'term':term,'session':session})
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                for kk in aalstu:
                    ...
                    co = StudentAcademicRecord.objects.get(student=kk, term = 'Third')
                    subclass=co.subclass
                    ...
                    if  old_class=='JS 3' or old_class=='SS 3':
                        ...
                        try:
                            ...
                            Student(birth_date= kk.birth_date,
                                ...
                                subclass = subclass,
                                userid = varuser,
                                studentpicture = kk.studentpicture).save()
