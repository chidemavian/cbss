...
# from utils import SchoolSessionValidator
...
class StudentAnnualRecord(models.Model):
    student = models.ForeignKey(Student)
    klass = models.CharField('Class', max_length=20, default='')
    arm = models.CharField('Arm', max_length=10, default='')
    session = models.CharField('Session', max_length=20, default='')
    annualaverage = models.CharField(max_length=20, default='')


    def __unicode__(self):
        return  u'Class: %s, Arm: %s,Session: %s' %(self.klass,self.arm,self.session)
...
class StudentAcademicRecord(models.Model):
    ...
    com1  = models.CharField('Classteacher Mid term Comment',max_length=3000, default='')
    class_teacher_comment = models.CharField("Class Teacher's End term Comment", max_length=3000, default='')

    com2 = models.CharField('principal mid term Comment',  max_length =3000, default='')
    principal_comment = models.CharField("Principal's End term  Comment", max_length=3000, default='')
...
class SubjectScore(models.Model):
    ...
    mid_term_score = models.CharField('Midterm', max_length=3, default=0)
    end_term_score = models.CharField('Term Score', max_length=3, default=0)
    ...
    remarks = models.CharField('Grade interpretation per subject', max_length=50, default='No entry')
    ...
    def save(self, **kwargs):
      ...
      if self.term.startswith('yT'):
          if self.klass.startswith('J'):
             if (self.annual_avg >=75) and (self.annual_avg <= 100):
                 self.grade = 'A'
                 self.remark = 'Excellent'
             elif (self.annual_avg >=65) and (self.annual_avg <= 74.99):
                 self.grade = 'B'
                 self.remark = 'Very Good'
             elif (self.annual_avg >=50) and (self.annual_avg <= 64.99):
                 self.grade = 'C'
                 self.remark = 'Good'
             ...
             super(SubjectScore, self).save(**kwargs)
              ...
              if (self.annual_avg >=80) and (self.annual_avg <= 100):
                  ...
                  self.remark = 'Commendable'
                   ...
                   self.grade = 'C5'
                   self.remark = 'Very Good'
              elif (self.annual_avg >=50) and (self.annual_avg <= 59.99):
                    self.grade = 'C6'
                    self.remark = 'Good'
              elif (self.annual_avg >=45) and (self.annual_avg <= 49.99):
                   self.grade = 'D7'
      ...
      if self.klass.startswith('J'):
            if (self.end_term_score >=75) and (self.end_term_score <=100 ):
                 self.grade = 'A'
                 self.remarks = 'EXCELLENT'

            elif (self.end_term_score >=65) and (self.end_term_score <= 74.99):
                 self.grade = 'B'
                 self.remarks = 'VERY GOOD'

            elif (self.end_term_score >=50) and (self.end_term_score <= 64.99):
                  self.grade = 'C'
                  self.remarks = 'GOOD'
            ...
            if (self.end_term_score >=80) and (self.end_term_score <= 100):
                self.grade = 'A1'
                self.remark = 'DISTINCTION'

            elif (self.end_term_score >=75) and (self.end_term_score <= 79.99):
                 self.grade = 'B2'
                 self.remark = 'EXCELLENT'

            elif (self.end_term_score >=70) and (self.end_term_score <= 74.99):
                 self.grade = 'B3'
                 self.remark = 'VERY GOOD'

            elif (self.end_term_score >=64) and (self.end_term_score <= 69.99):
                 self.grade = 'C4'
                 self.remark = 'VERY GOOD'
...
class Tblreportsheet(models.Model):
    student = models.ForeignKey(Student, related_name='repo')
    category= models.CharField('Category', max_length=20, )
    mid = models.CharField('Type', max_length=10, )
    end = models.CharField('Type', max_length=10, )
    score = models.CharField('Score', max_length=20, )
    session = models.CharField('Session', max_length=20, )
    term = models.CharField('Term', max_length=20,)
